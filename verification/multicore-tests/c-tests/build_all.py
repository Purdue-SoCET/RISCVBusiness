...
def compile(filename):
    ...
    rv = subprocess.run(compile_cmd + [filename, '-o', basename + '.elf'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
    if rv.returncode != 0:
        print('Exited with error {}, printing command, stdout, stderr!'.format(rv.returncode))
        print('Command: {}\n\n'.format(compile_cmd + [filename, '-o', basename + '.elf']))
        print('stdout:\n\n{}'.format(rv.stdout))
        print('stderr:\n\n{}'.format(rv.stderr))
        print('Exiting...')
        exit(1)

    print('Converting {} to binary'.format(filename))
