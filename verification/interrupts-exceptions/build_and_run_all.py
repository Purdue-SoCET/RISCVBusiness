...
testing_directory = os.path.abspath(os.path.join(current_directory, '..','..'))
...
for fname in (glob.glob('./*.c') + glob.glob('./*.S')):
    ...
    subprocess.run("./rvb_out/sim-verilator/Vtop_core verification/interrupts-exceptions/"+basename+".bin", shell=True)
    print("\n\n")
