...
the power and area overhead that a homogeneous design requires. Currently,
...
![
    Cache Incoherence Example.
    Each processor is depicted running the same program,
    explanation of execution follows.
](./fig/cache_incoherence.png)

Total program order in the example is used to
sequentialize operations ocurring in parallel. In this example, the total order consists of Processor 1 running all of its instructions, followed by Processor 2 running all of its instructions. Note that this interleaving is just one valid interleaving.
...
![
    Coherent system example. The same program is run as in the previous example, explanation follows.
](./fig/cache_coherence.png)
...
Additionally, the separate_caches.sv file in the source_code/caches/ directory can be
...
in this paper](https://ieeexplore.ieee.org/document/9218661).
