...
STAGE3_INC = $(PIPELINE)/stage3/include
STAGE3_SRC = $(PIPELINE)/stage3/source
RISCV_PKGS = $(RISCV)/packages
RISCV_PKGS_MGMT = $(RISCV_PKGS)/risc_mgmt
...
PIPELINE_FILES :=  $(PIPELINE)/stage3/source/stage3_execute_stage.sv $(PIPELINE)/tspp/tspp_execute_stage.sv  $(PIPELINE)/tspp/tspp_fetch_stage.sv  $(PIPELINE)/tspp/tspp_hazard_unit.sv  #$(PIPELINE)/tspp/tspp.sv
...
ALL_PKG = $(RISCV_PKGS_MGMT)/crc32_pkg.sv \
          $(RISCV_PKGS_MGMT)/template_pkg.sv \
          $(RISCV_PKGS_MGMT)/test_pkg.sv \
          $(RISCV_PKGS)/alu_types_pkg.sv \
          $(RISCV_PKGS)/machine_mode_types_1_12_pkg.sv \
          $(RISCV_PKGS)/machine_mode_types_1_11_pkg.sv \
          $(RISCV_PKGS)/machine_mode_types_1_7_pkg.sv \
          $(RISCV_PKGS)/machine_mode_types_pkg.sv \
          $(RISCV_PKGS)/pma_types_1_12_pkg.sv \
          $(RISCV_PKGS)/pmp_types_1_12_pkg.sv \
          $(RISCV_PKGS)/rv32a_pkg.sv \
          $(RISCV_PKGS)/rv32b_pkg.sv \
          $(RISCV_PKGS_MGMT)/rv32m_pkg.sv \
		  $(RISCV_PKGS)/rv32i_types_pkg.sv \
          $(RISCV)/pipelines/stage3/source/stage3_types_pkg.sv

ALL_INTERFACES = $(shell find $(RISCV) -type f -name "*.vh" ! -path "*/fpga/*" ! -path "*/uvm_tb/*" ! -path "*/tb/*" ! -path "*/tracker/*")
SRC_MODULES =  $(RV32C_FILES) $(RISC_MGMT_FILES) $(RISC_EXT_FILES) $(CORE_FILES) $(PIPELINE_FILES) $(PREDICTOR_FILES) $(PRIV_FILES) $(CACHE_FILES)  $(SPARCE_FILES) $(RISCV_BUS_FILES)
MC_MODULES = $(RISCV_CORE)/multicore_wrapper.sv
TB_INTERFACES = $(INTERFACE)/bus_if.vh $(INTERFACE)/multicore_if.vh 
TB_PKGS = $(PARAMS)/addr_space.sv $(PARAMS)/dut_parameters.sv

ALL_FILES = $(ALL_PKG) $(TB_PKGS) $(ALL_INTERFACES) $(TB_INTERFACES) $(SRC_MODULES) $(MC_MODULES)
