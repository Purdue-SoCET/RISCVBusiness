...
#include "Vrv32zc_alu.h"
#include "Vrv32zc_alu_rv32zc_pkg.h"
...
void tick(Vrv32zc_alu& dut, VerilatedFstC& trace) {
    dut.eval(); 
    trace.dump(sim_time++);
    dut.eval();
    trace.dump(sim_time++);
...
int main(int argc, char **argv) {
    ...
    struct test_vector_t {
        Vrv32zc_alu_rv32zc_pkg::rv32zc_op_t aluop;
        std::function<uint32_t(u32, u32)> eval;
    };

    static const test_vector_t ops[] = {
        {Vrv32zc_alu_rv32zc_pkg::EQZ, [](u32 a, u32 b) { 
            if (b != 0) return a; 
            else return 0; 
        }},
        {Vrv32zc_alu_rv32zc_pkg::NEZ, [](u32 a, u32 b) { 
            if (b == 0) return a; 
            else return 0; 
        }}
    };
